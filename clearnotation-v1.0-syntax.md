# ClearNotation v1.0 Syntax Decisions

This note freezes the remaining syntax choices that the v1.0 grammar depends on. It keeps the handoff baseline intact: readable-hybrid authoring, no inline HTML, inline-only notes, explicit directive blocks, trusted typed extensions in `clearnotation.toml`, and fail-closed behavior.

## Frozen v1.0 choices

- Canonical document extension: `.cln`
- Primary normative output target: HTML
- Secondary target status: PDF is supported, but only semantically conforming against the normalized AST; page layout is non-normative in v1.0
...
The grammar intentionally chooses a slightly smaller feature set for v1.0 to keep the syntax fully specified and predictably parsable.
...
`::meta{ ... }` is the only document-level metadata block in v1.0.
...
- Comments, multiline strings, inline tables, dates, and floats are not part of v1.0 `::meta`.
...
- Nested arrays and object literals are not part of v1.0
...
These are the intended built-in v1.0 signatures. `meta` is core syntax; the remaining entries belong to the compiler's core trusted registry and may be extended, but not redefined, by project config:
...
`::ref[target="..."]` is therefore explicitly part of the inline grammar in v1.0, while `::anchor[id="..."]` remains a block directive that attaches to the next addressable block.

The v1.0 set of addressable blocks is frozen in [clearnotation-v1.0-ast-conformance.md](/Users/ryan/projects/clear-notation/clearnotation-v1.0-ast-conformance.md).
...
- In raw bodies, a literal standalone `}` line is not directly representable in v1.0 and must be rewritten or split
...
The normalizer resolves `::include` directives recursively during the normalization phase:

- The target file is read, parsed, validated, and normalized recursively
- The resulting blocks replace the `::include` node in the normalized AST
- The target file's `::meta` is discarded; only the root document's metadata matters
- Circular includes produce a validation error
- Include depth is capped at 10 levels; exceeding this limit produces a normalization error
- Heading slug deduplication spans the merged document (slugs from included content participate in the root document's deduplication sequence)
- Note numbering continues across includes (notes in included content receive numbers following the root document's sequence in depth-first document order)
...
## What stays intentionally out of v1.0
...
That gives ClearNotation a fully specified syntax rather than a broad but underspecified one.
